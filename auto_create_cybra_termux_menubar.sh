...
echo "=== CREATE CYBRA TERMUX MENU-BAR ==="

mkdir -p \
  bin \
  parliament/committees/termux_menubar_committee \
  parliament/departments/finance_department/termux_menubar_committee \
  parliament/departments/cybra_finance_department/termux_menubar_committee \
  data/cybra_menubar/{reports,tasks,posts,withdraw_proposals,committees} \
  posts feeds proofs logs/cybra_menubar runtime/redis runtime
...
cat > parliament/committees/termux_menubar_committee/committee.json <<'JSON'
{
  "committee_id": "termux_menubar_committee",
  "name": "CYBRA Termux Menu-Bar Committee",
  "status": "active",
  "parent": "cybra_parliament",
  "mission": "Надати OWNER зручне Termux меню для керування CYBRA/KYBRA: токени, фінанси, withdraw proposals, AI tasks, пости, створення комітетів, статус виконання, зауваження, рекомендації, safe cycles.",
  "capabilities": [
    "show_tokens",
    "show_finance",
    "withdraw_proposal",
    "submit_ai_task",
    "create_parliament_post",
    "create_committee",
    "show_done_failed",
  ...
  "withdraw_rule": "Withdraw in menu creates proposal only. Real external transaction requires manual OWNER approval and valid wallet/payment rail.",
  "manual_OWNER_approval_required": true
}
JSON

cp parliament/committees/termux_menubar_committee/committee.json \
   parliament/departments/finance_department/termux_menubar_committee/committee.json 2>/dev/null || true

cp parliament/committees/termux_menubar_committee/committee.json \
   parliament/departments/cybra_finance_department/termux_menubar_committee/committee.json 2>/dev/null || true

cat > cybra_menubar.py <<'PY'
#!/usr/bin/env python3
import argparse
import json
import os
...
chmod +x cybra_menubar.py

cat > cybra_menubar.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
cd "$HOME/CYBRA"
...
chmod +x cybra_menubar.sh

cat > cybra_menubar_handler.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
cd "$HOME/CYBRA"
...
chmod +x cybra_menubar_handler.sh

redis-cli HSET cybra:executor:mapping menubar_owner_task cybra_menubar_handler.sh >/dev/null 2>&1 || true
redis-cli HSET cybra:executor:mapping menubar_post_task cybra_menubar_handler.sh >/dev/null 2>&1 || true
redis-cli HSET cybra:executor:mapping menubar_create_committee_task cybra_menubar_handler.sh >/dev/null 2>&1 || true
redis-cli HSET cybra:executor:mapping menubar_withdraw_proposal_task cybra_menubar_handler.sh >/dev/null 2>&1 || true
...
if p.exists():
    ...
    for line in [
        ...
        if line.strip().split(":")[0] not in s:
            i = s.find("SCRIPT_MAP")
            j = s.find("{", i)
            if i >= 0 and j >= 0:
                s = s[:j+1] + "\n" + line + s[j+1:]
...
python3 -m py_compile cybra_menubar.py
test -f parliament_executor_v6.py && python3 -m py_compile parliament_executor_v6.py || true
rm -rf __pycache__

ln -sf "$HOME/CYBRA/cybra_menubar.sh" "$PREFIX/bin/cybra-menu" 2>/dev/null || true
ln -sf "$HOME/CYBRA/cybra_menubar.sh" "$PREFIX/bin/cybra-menubar" 2>/dev/null || true

bash cybra_menubar.sh report
sha256sum -c proofs/cybra_menubar.sha256 || true

echo
echo "✅ CYBRA TERMUX MENU-BAR INSTALLED"
echo
echo "Run:"
echo "  cybra-menu"
echo "or:"
