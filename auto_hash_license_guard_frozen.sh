...
echo "=== CYBRA HASH LICENSE GUARD + FROZEN STATUS ==="

mkdir -p \
  parliament/departments/finance_department/hash_license_guard_committee \
  parliament/departments/cybra_finance_department/hash_license_guard_committee \
  data/hash_license_guard/{evidence,subjects,frozen,reports,tasks} \
  posts feeds proofs logs/hash_license_guard runtime/redis runtime
...
cat > parliament/departments/finance_department/hash_license_guard_committee/committee.json <<'JSON'
{
  "committee_id": "hash_license_guard_committee",
  "name": "Hash License Guard Committee",
  "parent_department": "finance_department",
  "status": "active",
  "mission": "Контролювати ліцензію Hash Module. Якщо аудит знаходить і підтверджує несанкціоноване комерційне використання — ставити внутрішній статус FROZEN.",
  "license_rule": "Unauthorized commercial use of the Hash Module is prohibited. If violation is detected and confirmed by audit, the subject is marked STATUS: FROZEN.",
  "frozen_scope": "Internal CYBRA/KYBRA status only: access flag, license flag, audit flag, commercial permission state.",
  "blocked_actions": [
    "no account seizure",
    "no external system interference",
    "no unlawful blocking",
  ...
  "requirements": [
    "audit_required",
    "evidence_required",
    "violation_confirmation_required",
    "manual_OWNER_review_required_for_unfreeze"
  ],
  "manual_OWNER_approval_required": true
}
JSON

cp parliament/departments/finance_department/hash_license_guard_committee/committee.json \
   parliament/departments/cybra_finance_department/hash_license_guard_committee/committee.json 2>/dev/null || true

cat > data/hash_license_guard/license_policy.json <<'JSON'
{
  "policy_id": "CYBRA-HASH-LICENSE-POLICY",
  "owner_license": "OWNER_CONTROLLED_HASH_MODULE_LICENSE",
  "rule": "If unauthorized commercial use of my Hash Module is found and confirmed by audit, the violating entity is marked STATUS: FROZEN.",
  "violation_types": [
    "unauthorized_commercial_use",
    "copying_without_permission",
    "integration_without_license",
    "paid_product_use_without_owner_approval",
    "commercial_api_backend_use_without_license",
    "redistribution_without_permission",
    "monetization_without_owner_approval"
  ],
  "frozen_meaning": [
    ...
    "unfreeze only after OWNER approval"
  ],
  "safety": {
...
chmod +x hash_license_guard.py

cat > hash_license_guard.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x hash_license_guard.sh

cat > hash_license_guard_handler.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x hash_license_guard_handler.sh

redis-cli HSET cybra:executor:mapping hash_license_violation_audit_task hash_license_guard_handler.sh >/dev/null || true
...
if p.exists():
    ...
    if '"hash_license_violation_audit_task"' not in s:
        i = s.find("SCRIPT_MAP")
        j = s.find("{", i)
        if i >= 0 and j >= 0:
            s = s[:j+1] + '\n    "hash_license_violation_audit_task": "hash_license_guard_handler.sh",' + s[j+1:]
...
python3 -m py_compile hash_license_guard.py
test -f parliament_executor_v6.py && python3 -m py_compile parliament_executor_v6.py || true
rm -rf __pycache__

echo
echo "=== CREATE AI TASK + FIRST REPORT ==="
bash hash_license_guard.sh submit-ai
bash hash_license_guard.sh report
...
echo
echo "=== STATUS ==="
bash hash_license_guard.sh status
sha256sum -c proofs/hash_license_guard.sha256 || true

echo
echo "✅ HASH LICENSE GUARD INSTALLED"
