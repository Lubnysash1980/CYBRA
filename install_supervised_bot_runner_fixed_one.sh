...
echo "=== CYBRA SUPERVISED BOT RUNNER FIXED ONE ==="
echo "Mode: PAPER / TESTNET / IT + CYBERPARLIAMENT"
echo "Live orders: BLOCKED"

mkdir -p scripts/bot_supervisor \
  data/cybra_bot_supervisor/reports \
  data/cybra_bot_supervisor/logs \
  data/cybra_bot_supervisor/pids \
  data/cybra_bot_supervisor/audit \
  data/cybra_bot_supervisor/tasks \
  data/cybra_bot_supervisor/watchdog \
  data/cybra_bot_supervisor/harness \
...
cat > scripts/bot_supervisor/cybra_supervised_bot_runner_fixed.py <<'PY'
#!/usr/bin/env python3
...
#!/usr/bin/env bash
cd "$HOME/CYBRA" || exit 1
python3 scripts/bot_supervisor/cybra_supervised_bot_runner_fixed.py "$@"
SH

chmod +x scripts/bot_supervisor/cybra_supervised_bot_runner_fixed.py cybra-bot-supervised
ln -sf "$HOME/CYBRA/cybra-bot-supervised" "$PREFIX/bin/cybra-bot-supervised" 2>/dev/null || true

echo
echo "=== START SUPERVISED PAPER HARNESS ==="
cybra-bot-supervised start
...
echo
echo "=== GIT SAFE ADD / COMMIT / PUSH ==="
git add -f \
  install_supervised_bot_runner_fixed_one.sh \
  scripts/bot_supervisor/cybra_supervised_bot_runner_fixed.py \
  cybra-bot-supervised \
  data/cybra_bot_supervisor/reports \
  data/cybra_bot_supervisor/audit \
  data/cybra_bot_supervisor/tasks \
  data/cybra_bot_supervisor/watchdog \
  data/cybra_bot_supervisor/harness \
  data/cybra_bar/menus/bot_supervisor_menu.json \
  data/cybra_finance/it_department/tasks \
  parliament/inbox \
