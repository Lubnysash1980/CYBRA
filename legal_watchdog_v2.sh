...
mkdir -p legal/watchdog legal/responses legal/autofix posts proofs

cat > legal_watchdog_v2.py <<'PY'
...
python3 legal_watchdog_v2.py

git add legal_watchdog_v2.py legal/watchdog posts proofs legal_watchdog_v2.sh
git commit -m "add legal watchdog v2 for submissions and formal replies" || true

echo "✅ Legal watchdog v2 created"
