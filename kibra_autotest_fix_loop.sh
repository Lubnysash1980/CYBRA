...
echo "=== KIBRA AUTO TEST → FIX → TEST LOOP ==="
...
mkdir -p \
  ...
  blockchain/kibra_chain/mainnet/mempool \
  data/cybra_mainnet/{audit,claims,live,miners,node,reports,manifests} \
  dashboard/kibra_mainnet \
  posts feeds proofs logs/mainnet

cat > scripts/kibra/kibra_autotest_fix_loop.py <<'PY'
#!/usr/bin/env python3
...
chmod +x scripts/kibra/kibra_autotest_fix_loop.py

cat > cybra-kibra-loop <<'EOF'
#!/usr/bin/env bash
cd "$HOME/CYBRA" || exit 1
...
chmod +x cybra-kibra-loop
ln -sf "$HOME/CYBRA/cybra-kibra-loop" "$PREFIX/bin/cybra-kibra-loop" 2>/dev/null || true

echo
echo "=== RUN LOOP ==="
KIBRA_WALLET="$WALLET" cybra-kibra-loop run "$CYCLES"

echo
echo "=== FINAL STATUS ==="
cybra-kibra-loop status

echo
echo "=== PROOF ==="
cybra-kibra-loop proof

echo
echo "✅ KIBRA AUTO TEST/FIX LOOP DONE"
