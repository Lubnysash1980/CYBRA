...
echo "=== CREATE KIBRA MARKET PROOF ENGINE ==="

mkdir -p \
  data/kibra_market_proof/evidence \
  data/kibra_market_proof/reports \
  data/kibra_market_proof/private \
  posts feeds proofs logs/market_proof runtime/redis

chmod 700 data/kibra_market_proof/private 2>/dev/null || true
...
cat > kibra_market_proof_engine.py <<'PY'
#!/usr/bin/env python3
...
chmod +x kibra_market_proof_engine.py

cat > kibra_market_proof.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
cd "$HOME/CYBRA" || exit 1
...
chmod +x kibra_market_proof.sh
ln -sf "$HOME/CYBRA/kibra_market_proof.sh" "$PREFIX/bin/kibra-market-proof" 2>/dev/null || true

echo "=== INIT MARKET PROOF ==="
kibra-market-proof init

echo
echo "=== VERIFY PROOF ==="
sha256sum -c proofs/kibra_market_proof.sha256

echo
echo "✅ Market Proof Engine installed"
echo
echo "Commands:"
echo "  kibra-market-proof status"
echo "  kibra-market-proof add PROVIDER PAIR PRICE LIQUIDITY_USD VOLUME_24H_USD SOURCE_URL"
echo "  kibra-market-proof verify"
echo "  kibra-market-proof push"
