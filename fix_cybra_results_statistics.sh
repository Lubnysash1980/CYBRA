...
echo "=== REDIS RESULT KEYS ==="
echo "cybra:results: $(redis-cli LLEN cybra:results 2>/dev/null || echo 0)"
echo "cybra:parliament:results: $(redis-cli LLEN cybra:parliament:results 2>/dev/null || echo 0)"
echo "cybra:parliament:failed: $(redis-cli LLEN cybra:parliament:failed 2>/dev/null || echo 0)"

echo
echo "=== PATCH CYBRA CLI RESULT KEY ==="

if [ -f "$PREFIX/bin/cybra" ]; then
  sed -i 's/cybra:results/cybra:parliament:results/g' "$PREFIX/bin/cybra"
  chmod +x "$PREFIX/bin/cybra"
  echo "✅ patched $PREFIX/bin/cybra"
fi

if [ -f "./cybra" ]; then
  sed -i 's/cybra:results/cybra:parliament:results/g' "./cybra"
  chmod +x "./cybra"
  echo "✅ patched ./cybra"
fi

cat > cybra_task_statistics.sh <<'STAT'
#!/data/data/com.termux/files/usr/bin/bash
set -e
...
chmod +x cybra_task_statistics.sh

echo
echo "=== RUN FIXED STATISTICS ==="
bash cybra_task_statistics.sh

echo
echo "=== CYBRA CLI CHECK ==="
cybra status || true
cybra results | head -5 || true
