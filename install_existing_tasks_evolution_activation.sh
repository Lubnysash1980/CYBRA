...
echo "=== INSTALL EXISTING TASKS EVOLUTION ACTIVATION ==="

mkdir -p \
  parliament/existing_tasks_activation \
  parliament/committees \
  posts feeds proofs logs/existing_tasks
...
cat > parliament/existing_tasks_activation/policy.json <<'JSON'
{
  "name": "CYBRA Existing Tasks Evolution Activation",
  "status": "active",
  "mode": "safe_mapping_repair_validation",
  "purpose": "Перевірити всі існуючі задачі, створити mapping/committee/handler fallback і підключити їх до Evolution Deployment.",
  "rules": {
    "do_not_execute_real_payments": true,
    "do_not_execute_real_token_mint": true,
    "do_not_execute_external_blockchain_tx": true,
    "owner_manual_approval_required": true,
    "archive_old_fixed_failures": true,
    "create_missing_committees": true,
    "create_missing_mapping": true,
    "validate_handlers": true
  }
}
JSON

cat > generic_existing_task_handler.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x generic_existing_task_handler.sh

cat > cybra_existing_tasks_evolution_activation.py <<'PY'
#!/usr/bin/env python3
import json
import time
import hashlib
...
chmod +x cybra_existing_tasks_evolution_activation.py

cat > existing_tasks_activation_handler.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x existing_tasks_activation_handler.sh

cat > cybra_existing_tasks.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x cybra_existing_tasks.sh

redis-cli HSET cybra:executor:mapping existing_tasks_activation_task existing_tasks_activation_handler.sh >/dev/null
...
if '"existing_tasks_activation_task"' not in s:
    i = s.find("SCRIPT_MAP")
    j = s.find("{", i)
    if i >= 0 and j >= 0:
        s = s[:j+1] + '\n    "existing_tasks_activation_task": "existing_tasks_activation_handler.sh",' + s[j+1:]

p.write_text(s)
print("✅ existing_tasks_activation_task mapping ready")
PY

rm -rf __pycache__
python3 -m py_compile parliament_executor_v6.py
python3 -m py_compile cybra_existing_tasks_evolution_activation.py
rm -rf __pycache__

echo
echo "=== 1. REPAIR EXISTING TASKS ==="
bash cybra_existing_tasks.sh repair

echo
echo "=== 2. RUN ACTIVATION CYCLE ==="
bash cybra_existing_tasks.sh cycle

echo
echo "=== 3. FINAL STATUS ==="
bash cybra_existing_tasks.sh status
cybra status || true

echo
echo "=== 4. PROOF CHECK ==="
sha256sum -c proofs/existing_tasks_evolution_activation.sha256

echo
echo "✅ EXISTING TASKS EVOLUTION ACTIVATION INSTALLED"
