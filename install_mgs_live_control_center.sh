...
echo "=== INSTALL MGS LIVE CONTROL CENTER ==="

mkdir -p \
  bin scripts dashboard/mgs .github/workflows .devcontainer \
  data/cybra_mgs/{tasks,reports,codespace,committees,evolution,metrics} \
  posts feeds proofs logs/mgs runtime/redis
...
chmod +x cybra_mgs_live.py

# ---------- CLI ----------
cat > cybra-mgs-live <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
cd "$HOME/CYBRA" || exit 1
python3 cybra_mgs_live.py "$@"
EOF

chmod +x cybra-mgs-live
ln -sf "$HOME/CYBRA/cybra-mgs-live" "$PREFIX/bin/cybra-mgs-live"

# ---------- Codespace worker ----------
cat > scripts/cybra_codespace_mgs_live_worker.py <<'PY'
#!/usr/bin/env python3
...
chmod +x scripts/cybra_codespace_mgs_live_worker.py

# ---------- GitHub Action ----------
cat > .github/workflows/cybra-mgs-live-worker.yml <<'YAML'
name: CYBRA MGS Live Worker
...
on:
  ...
  push:
    paths:
      ...
      - 'scripts/cybra_codespace_mgs_live_worker.py'
...
jobs:
  run-mgs-worker:
    ...
    steps:
      ...
      - name: Run MGS Codespace/GitHub worker
        run: python3 scripts/cybra_codespace_mgs_live_worker.py
      - name: Commit worker reports
        run: |
          git config user.name "cybra-mgs-live-bot"
          git config user.email "cybra-mgs-live-bot@users.noreply.github.com"
          git add data/cybra_mgs/codespace feeds/cybra_mgs_codespace_worker.json posts/cybra_mgs_codespace_worker.md proofs/cybra_mgs_codespace_worker.sha256 || true
          git commit -m "update MGS live worker reports" || true
          git push || true
YAML

# ---------- Devcontainer start ----------
cat > .devcontainer/cybra_mgs_live_start.sh <<'BASH2'
#!/usr/bin/env bash
set -e
...
chmod +x .devcontainer/cybra_mgs_live_start.sh

if [ ! -f .devcontainer/devcontainer.json ]; then
cat > .devcontainer/devcontainer.json <<'JSON'
{
  "name": "CYBRA MGS Live Codespace",
  "image": "mcr.microsoft.com/devcontainers/python:3.12",
  "postCreateCommand": "bash .devcontainer/cybra_mgs_live_start.sh"
}
JSON
fi

# ---------- initial status ----------
python3 cybra_mgs_live.py status

sha256sum \
  cybra_mgs_live.py \
  cybra-mgs-live \
  scripts/cybra_codespace_mgs_live_worker.py \
  .github/workflows/cybra-mgs-live-worker.yml \
  .devcontainer/cybra_mgs_live_start.sh \
  dashboard/mgs/index.html \
  feeds/cybra_mgs_live_metrics.json \
  posts/cybra_mgs_live_status.md \
  > proofs/cybra_mgs_live_install.sha256

echo "✅ MGS LIVE CONTROL CENTER INSTALLED"
echo "Command: cybra-mgs-live status"
echo "Command: cybra-mgs-live serve"
echo "Dashboard: http://127.0.0.1:8765/dashboard/mgs/"
