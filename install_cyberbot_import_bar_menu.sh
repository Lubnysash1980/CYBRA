...
echo "=== INSTALL CYBERBOT IMPORT BAR MENU ==="

mkdir -p \
  scripts/cyberbot \
  data/cyberbot/{reports,actions,tasks,audit,committees,patches,imports,config} \
  data/cybra_finance/it_department/tasks \
  data/cybra_bar/menus \
  parliament/inbox parliament/committees \
  posts feeds proofs dashboard/cyberbot runtime/redis
...
grep -qxF "data/cyberbot/patches/*.tar.gz" .gitignore 2>/dev/null || echo "data/cyberbot/patches/*.tar.gz" >> .gitignore

cat > scripts/cyberbot/cyberbot.py <<'PY'
#!/usr/bin/env python3
...
#!/usr/bin/env bash
cd "$HOME/CYBRA" || exit 1
python3 scripts/cyberbot/cyberbot.py "$@"
SH
...
cat > cyberbot-menu <<'SH'
#!/usr/bin/env bash
cd "$HOME/CYBRA" || exit 1
python3 scripts/cyberbot/cyberbot.py menu
SH

chmod +x scripts/cyberbot/cyberbot.py cyberbot Cyberbot cyberbot-menu
ln -sf "$HOME/CYBRA/cyberbot" "$PREFIX/bin/cyberbot" 2>/dev/null || true
ln -sf "$HOME/CYBRA/Cyberbot" "$PREFIX/bin/Cyberbot" 2>/dev/null || true
ln -sf "$HOME/CYBRA/cyberbot-menu" "$PREFIX/bin/cyberbot-menu" 2>/dev/null || true

python3 scripts/cyberbot/cyberbot.py import-file
cyberbot status
cyberbot proof
