...
echo "=== INSTALL KIBRA REAL INTERNAL MAINNET NODE ==="
...
mkdir -p \
  ...
  data/cybra_mainnet/node \
  data/cybra_mainnet/reports \
  data/cybra_mainnet/audit \
  dashboard/kibra_mainnet \
  posts feeds proofs logs/mainnet

cat > scripts/kibra/cybra_kibra_real_node.py <<'PY'
#!/usr/bin/env python3
...
chmod +x scripts/kibra/cybra_kibra_real_node.py
...
echo
echo "=== RUN NODE TEST ==="
KIBRA_WALLET="$WALLET" cybra-kibra-real test

echo
echo "✅ KIBRA REAL INTERNAL NODE INSTALLED"
echo
echo "Next:"
echo "  cybra-kibra-real status"
echo "  cybra-kibra-real validate"
