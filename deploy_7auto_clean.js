...
// ============================================================
// РІВЕНЬ 2: ПЕРЕВІРКА ТА ОЧИЩЕННЯ БАЙТКОДУ
// ============================================================
async function level2_bytecode() {
  log('=== РІВЕНЬ 2: ПЕРЕВІРКА ТА ОЧИЩЕННЯ БАЙТКОДУ ===');
  ...
  // Очищаємо від усього, крім hex-цифр (після 0x)
  const hexPart = code.slice(2).replace(/[^0-9a-fA-F]/g, '');
  code = '0x' + hexPart;
  fs.writeFileSync('./contracts/CybraToken.bin', code);
  if (code.length < 1000) warn('Байткод занадто короткий: ' + code.length);
  ok('Байткод завантажено та очищено, довжина: ' + code.length);
  return code;
}

// ============================================================
// РІВЕНЬ 3: ПІДКЛЮЧЕННЯ ДО RPC
// ============================================================
async function level3_rpc() {
  ...
  let blockNumber;
  try { blockNumber = await provider.getBlockNumber(); } catch (e) {
  ...
  try { nonce = await wallet.getTransactionCount(); } catch (e) {
    error('Не вдалося отримати nonce:', e.message);
    process.exit(1);
  }
  ok('nonce: ' + nonce);
  try { gasPrice = await provider.getGasPrice(); } catch (e) {
...
// ============================================================
// РІВЕНЬ 5: ПЕРЕВІРКА HEXLIFY (З ВИНЯТКОМ ДЛЯ DATA)
// ============================================================
async function level5_hexlify_check() {
  ...
  for (const [key, value] of Object.entries(tx)) {
    if (key === 'data') {
      // Для data перевіряємо, чи це валідний hex-рядок
      if (typeof value === 'string' && value.startsWith('0x') && /^[0-9a-fA-F]+$/.test(value.slice(2))) {
        ok('Поле data вже є валідним hex-рядком.');
      } else {
        // Спроба очистити
        const clean = '0x' + String(value).slice(2).replace(/[^0-9a-fA-F]/g, '');
        if (/^[0-9a-fA-F]+$/.test(clean.slice(2))) {
          tx.data = clean;
          ok('Поле data очищено та виправлено.');
        } else {
          error('Не вдалося виправити поле data');
          allOk = false;
        }
      }
    } else {
...
async function level7_send() {
  ...
  try {
    ...
    if (!receipt) {
      error('Тайм-аут. Перевірте хеш: ' + response.hash);
      process.exit(1);
    }
    if (receipt.status === 1) {
...
// ============================================================
async function main() {
  console.log('🛠️  АВТОМАТИЧНИЙ ДЕПЛОЙ З 7 РІВНЯМИ (ОЧИЩЕННЯ)');
  try {
    await level1_env();
