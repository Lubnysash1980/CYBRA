...
echo "=== INSTALL KIBRA MARKET PRICE PATHS ==="

mkdir -p \
  parliament/committees/kibra_market_price_confirmation_committee/paths \
  parliament/departments/kibra_mint_repair_department/price_confirmation_committee/market_paths_department \
  ...
  posts feeds proofs logs/kibra_market_paths
...
cat > parliament/departments/kibra_mint_repair_department/price_confirmation_committee/market_paths_department/department.json <<'JSON'
{
  "department_id": "kibra_market_price_paths_department",
  "name": "KIBRA Market Price Paths Department",
  "parent_committee": "kibra_market_price_confirmation_committee",
  "status": "active",
  "mission": "Підготувати реальні можливі шляхи підтвердження ринкової ціни KIBRA: DEX pool, orderbook/backend provider, reserve-backed peg, external blockchain anchor.",
  "paths": [
    "DEX_POOL_READINESS",
    "ORDERBOOK_BACKEND_PROVIDER",
    "RESERVE_BACKED_PEG",
    "EXTERNAL_BLOCKCHAIN_ANCHOR"
  ],
  "rule": "Internal blockchain and external anchor prove blocks/shares/hash. Market price is confirmed only by DEX pool/orderbook/provider/reserve-backed peg proof.",
  "blocked": [
    "fake_price",
    "fake_volume",
    "wash_trading",
    "guaranteed_profit",
    "automatic_real_sell",
    "automatic_external_tx_without_OWNER_approval"
  ],
  "manual_OWNER_approval_required": true
}
JSON

cat > cybra_kibra_market_paths.py <<'PY'
#!/usr/bin/env python3
...
chmod +x cybra_kibra_market_paths.py

cat > kibra_market_price_paths_handler.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x kibra_market_price_paths_handler.sh

cat > cybra_market_paths.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x cybra_market_paths.sh

redis-cli HSET cybra:executor:mapping kibra_market_price_paths_task kibra_market_price_paths_handler.sh >/dev/null
...
if '"kibra_market_price_paths_task"' not in s:
    i = s.find("SCRIPT_MAP")
    j = s.find("{", i)
    if i >= 0 and j >= 0:
        s = s[:j+1] + '\n    "kibra_market_price_paths_task": "kibra_market_price_paths_handler.sh",' + s[j+1:]

p.write_text(s)
print("✅ kibra_market_price_paths_task mapping ready")
PY

rm -rf __pycache__
python3 -m py_compile cybra_kibra_market_paths.py
python3 -m py_compile parliament_executor_v6.py
rm -rf __pycache__

echo
echo "=== CREATE MARKET PRICE PATHS + AI TASKS ==="
bash cybra_market_paths.sh submit-ai

echo
echo "=== RUN CYCLE ==="
bash cybra_market_paths.sh cycle || true

echo
echo "=== STATUS ==="
bash cybra_market_paths.sh status

echo
echo "=== PROOF CHECK ==="
sha256sum -c proofs/kibra_market_price_paths.sha256 || true

echo
echo "✅ KIBRA MARKET PRICE PATHS INSTALLED"
