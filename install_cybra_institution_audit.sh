...
echo "=== CYBRA INSTITUTION AUDIT INSTALL ==="
...
cat > cybra_institution_audit.py <<'PY'
#!/usr/bin/env python3
import json
import time
import hashlib
...
chmod +x cybra_institution_audit.py

cat > institution_audit_handler.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x institution_audit_handler.sh

cat > cybra_institution.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x cybra_institution.sh

redis-cli HSET cybra:executor:mapping institution_audit_task institution_audit_handler.sh >/dev/null
...
if '"institution_audit_task"' not in s:
    i = s.find("SCRIPT_MAP")
    j = s.find("{", i)
    if i >= 0 and j >= 0:
        s = s[:j+1] + '\n    "institution_audit_task": "institution_audit_handler.sh",' + s[j+1:]

p.write_text(s)
print("✅ institution_audit_task mapping ready")
PY
...
echo
echo "=== 1. CHECK ==="
bash cybra_institution.sh check

echo
echo "=== 2. REPAIR / CREATE MISSING STRUCTURE ==="
bash cybra_institution.sh repair

echo
echo "=== 3. TASK TEST ==="
bash cybra_institution.sh task
cybra worker-start || true
sleep 8

echo
echo "=== 4. STATUS ==="
bash cybra_institution.sh status
cybra status
