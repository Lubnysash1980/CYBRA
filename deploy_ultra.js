...
async function main() {
  ...
  // --- ВРУЧНЕ ПЕРЕТВОРЕННЯ ВСІХ ПОЛІВ У HEX ---
  const tx = {
    data: ethers.utils.hexlify(bytecode),           // явно hexlify
    gasLimit: ethers.utils.hexlify(3000000),        // hex-рядок
    chainId: ethers.utils.hexlify(56),              // hex-рядок
    nonce: ethers.utils.hexlify(nonce),             // hex-рядок
    gasPrice: ethers.utils.hexlify(gasPrice)        // hex-рядок
  };

  console.log('🔹 Всі поля перетворено в hex-рядки');
  ...
  let receipt = null;
  for (let i = 0; i < 24; i++) { // 24 спроби по 5 секунд = 2 хвилини
  ...
  if (!receipt) {
    console.error('❌ Тайм-аут очікування підтвердження. Перевірте статус на BSCScan за хешем:', response.hash);
    process.exit(1);
  }
