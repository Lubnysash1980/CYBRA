...
echo "=== INSTALL KIBRA MINT AUDIT DEPARTMENT ==="

mkdir -p \
  parliament/departments/kibra_mint_repair_department/audit_department \
  parliament/kibra_mint_audit \
  data/kibra_mint_audit \
  posts feeds proofs logs/kibra_mint_audit
...
cat > parliament/departments/kibra_mint_repair_department/audit_department/department.json <<'JSON'
{
  "department_id": "kibra_mint_audit_department",
  "name": "KIBRA Mint Audit Department",
  "parent_department": "kibra_mint_repair_department",
  "status": "active",
  "mission": "Аудит монетного двору KIBRA: перевірка native coin, task-blocks, pool mining, repair queue, bridge, promotion, difficulty classes, price/sell proposals, proof та AI-завдань.",
  "checks": [
    "native_kibra_package",
    "kibra_chain_blocks",
    "pool_confirmed_blocks",
  ...
  "limits": [
    ...
    "no_external_tx_without_OWNER_approval",
    "manual_OWNER_approval_required"
  ]
}
JSON

cat > parliament/kibra_mint_audit/policy.json <<'JSON'
{
  "name": "KIBRA Mint Audit Policy",
  "status": "active",
  "mode": "audit_detect_repair_recommend_ai_tasks",
  "native_coin": true,
  "external_mint": false,
  "rule": "Монетний двір не просто створює блоки. Аудит-відділ перевіряє, що блоки мають proof, pool attribution, AI task payload, difficulty class, bridge package і repair route.",
  "audit_targets": [
    "KIBRA native coin",
    "KIBRA blocks",
    "AI task mining blocks",
    "pool mining",
    "mint repair",
    "promotion",
    "difficulty classes",
...
cat > cybra_kibra_mint_audit.py <<'PY'
#!/usr/bin/env python3
import json
import time
import hashlib
...
chmod +x cybra_kibra_mint_audit.py

cat > kibra_mint_audit_handler.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x kibra_mint_audit_handler.sh

cat > cybra_mint_audit.sh <<'EOF2'
#!/data/data/com.termux/files/usr/bin/bash
set -e
cd "$HOME/CYBRA"
...
chmod +x cybra_mint_audit.sh

redis-cli HSET cybra:executor:mapping kibra_mint_audit_task kibra_mint_audit_handler.sh >/dev/null
...
if '"kibra_mint_audit_task"' not in s:
    i = s.find("SCRIPT_MAP")
    j = s.find("{", i)
    if i >= 0 and j >= 0:
        s = s[:j+1] + '\n    "kibra_mint_audit_task": "kibra_mint_audit_handler.sh",' + s[j+1:]

p.write_text(s)
print("✅ kibra_mint_audit_task mapping ready")
PY

rm -rf __pycache__
python3 -m py_compile cybra_kibra_mint_audit.py
python3 -m py_compile parliament_executor_v6.py
rm -rf __pycache__

echo
echo "=== CREATE MINT AUDIT REPORT + AI TASKS ==="
bash cybra_mint_audit.sh submit-ai

echo
echo "=== ADD MASTER TASK TO PARLIAMENT ==="
bash cybra_mint_audit.sh task
...
echo
echo "=== STATUS ==="
bash cybra_mint_audit.sh status

echo
echo "=== PROOF CHECK ==="
sha256sum -c proofs/kibra_mint_audit.sha256 || true

echo
echo "✅ KIBRA MINT AUDIT DEPARTMENT INSTALLED"
