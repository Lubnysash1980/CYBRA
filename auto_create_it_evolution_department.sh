...
echo "=== CREATE CYBRA IT + EVOLUTION DEPARTMENT ==="

mkdir -p \
  bin \
  parliament/committees/evolution_committee \
  parliament/committees/evolution_committee/subcommittees/evolution_binary_subcommittee \
  parliament/committees/it_department \
  parliament/committees/it_department/subcommittees/programming_subcommittee \
  parliament/committees/it_department/subcommittees/analytics_subcommittee \
  parliament/committees/it_department/subcommittees/security_subcommittee \
  parliament/committees/it_department/subcommittees/refactor_subcommittee \
  parliament/committees/it_department/subcommittees/cleanup_subcommittee \
  parliament/committees/it_department/subcommittees/script_executor_subcommittee \
  parliament/committees/runtime_continuity_subcommittee \
  data/cybra_it_evolution/{registry,tasks,reports,plans,proofs,watchdog,logs} \
  posts feeds proofs logs/it_evolution runtime/redis runtime
...
cat > parliament/committees/evolution_committee/committee.json <<'JSON'
{
  "committee_id": "evolution_committee",
  "name": "CYBRA Evolution Committee",
  "status": "active",
  "parent": "cybra_parliament",
  "mission": "Щоденно вимірювати, планувати і запускати еволюцію CYBRA/KYBRA: що покращилось, що стоїть, що треба доробити, які задачі на день.",
  "responsibilities": [
    "daily_evolution_score",
    "daily_goal_plan",
    "task_blocks_growth",
    "module_readiness",
    "blocker_detection",
    "tomorrow_recommendations",
    "evolution_binary_control"
  ],
  "rule": "Кожен день має бути evolution snapshot: score %, delta %, blockers, next tasks.",
  "manual_OWNER_approval_required": true
}
JSON

cat > parliament/committees/evolution_committee/subcommittees/evolution_binary_subcommittee/committee.json <<'JSON'
{
  "committee_id": "evolution_binary_subcommittee",
  "name": "CYBRA Evolution Binary Subcommittee",
  "status": "active",
  "parent": "evolution_committee",
  "mission": "Створювати власний CYBRA binary/command center, який підключає існуючі скрипти, додає нові команди, запускає safe cycles, створює reports/proofs.",
  "capabilities": [
    "script_registry",
    "command_aliases",
    "safe_runner",
...
cat > parliament/committees/it_department/committee.json <<'JSON'
{
  "committee_id": "it_department",
  "name": "CYBRA IT Department",
  "status": "active",
  "parent": "cybra_parliament",
  "mission": "Великий IT-відділ для програмування, аналітики, безпеки, cleanup, refactor, написання скриптів, виконання задач, тестування і підтримки еволюції.",
  "subcommittees": [
    "programming_subcommittee",
    "analytics_subcommittee",
    "security_subcommittee",
    "refactor_subcommittee",
    "cleanup_subcommittee",
    "script_executor_subcommittee"
  ],
  "workflow": "task -> analysis -> script -> test -> report -> proof -> parliament/task-block",
  "manual_OWNER_approval_required": true
}
JSON

cat > parliament/committees/runtime_continuity_subcommittee/committee.json <<'JSON'
{
  "committee_id": "runtime_continuity_subcommittee",
  "name": "CYBRA Runtime Continuity Subcommittee",
  "status": "active",
  "parent": "cybra_parliament",
  "mission": "Стежити, щоб процеси CYBRA не зупинялися раптово, не ламали одне одного, перезапускали Redis, AutoHeal, Security, Conformation, Dashboard, Recovery, Codespace Runtime.",
  "watchdog_policy": [
    "check_redis",
    "check_reports",
    "run_safe_cycles",
    "avoid_parallel_git_lock",
    "avoid_destructive_cleanup",
    "restart_only_safe_processes"
  ],
  "blocked": [
    "killing_unknown_user_processes",
...
for sub in programming_subcommittee analytics_subcommittee security_subcommittee refactor_subcommittee cleanup_subcommittee script_executor_subcommittee; do
cat > "parliament/committees/it_department/subcommittees/$sub/committee.json" <<JSON
{
  "committee_id": "$sub",
  "name": "CYBRA ${sub}",
  "status": "active",
  "parent": "it_department",
  "mission": "Part of CYBRA IT Department: $sub.",
  "safe_mode": true,
  "manual_OWNER_approval_required": true
}
JSON
...
chmod +x cybra_it_evolution.py

cat > cybra_it_evolution.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
cd "$HOME/CYBRA" || exit 1
python3 cybra_it_evolution.py "$@"
...
chmod +x cybra_it_evolution.sh

cat > cybra_it_evolution_handler.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
...
chmod +x cybra_it_evolution_handler.sh

redis-cli HSET cybra:executor:mapping it_evolution_task cybra_it_evolution_handler.sh >/dev/null 2>&1 || true

ln -sf "$HOME/CYBRA/cybra_it_evolution.sh" "$PREFIX/bin/cybra-it" 2>/dev/null || true
ln -sf "$HOME/CYBRA/cybra_it_evolution.sh" "$PREFIX/bin/cybra-evo-it" 2>/dev/null || true
...
print("✅ menubar patched with it command")
PY
fi

python3 -m py_compile cybra_it_evolution.py
rm -rf __pycache__ 2>/dev/null || true

bash cybra_it_evolution.sh cycle
sha256sum -c proofs/cybra_it_evolution.sha256 || true

echo
echo "✅ CYBRA IT + EVOLUTION DEPARTMENT CREATED"
echo
echo "Commands:"
echo "  cybra-it status"
echo "  cybra-it plan"
echo "  cybra-it task 'Написати модуль ...'"
echo "  cybra-it run autoheal"
echo "  cybra-it watchdog"
echo "  cybra-it clean"
echo "  cybra-it cycle"
echo "  bin/cybra-evolution-bin status"
