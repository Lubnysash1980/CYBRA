...
echo "=== CREATE CYBRA BACKGROUND PLATFORM ==="

mkdir -p runtime/cybra_bg/pids runtime/cybra_bg/locks runtime/redis logs/background posts feeds proofs data/cybra_background/reports bin

cat > cybra_bg_worker_runner.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set +e
...
chmod +x cybra_bg_worker_runner.sh
...
ln -sf "$HOME/CYBRA/cybra_bg.sh" "$PREFIX/bin/cybra-bg" 2>/dev/null || true
ln -sf "$HOME/CYBRA/cybra_bg.sh" "$PREFIX/bin/cybra-background" 2>/dev/null || true

python3 -m py_compile cybra_bg_worker_runner.sh 2>/dev/null || true

echo
echo "✅ CYBRA BACKGROUND PLATFORM CREATED"
echo
echo "Run:"
echo "  cybra-bg start"
echo "  cybra-bg status"
